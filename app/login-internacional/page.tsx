export default function LoginInternacionalPage({
  searchParams,
}: {
  searchParams: { next?: string; error?: string };
}) {
  const next = searchParams.next ?? "/internacional";
  ...
  return (
    <div className="flex min-h-[70vh] items-center justify-center px-4">
      <form
        method="POST"
        action="/api/login-internacional"
        className="w-full max-w-sm space-y-4 rounded-lg border bg-background p-6 shadow-sm"
      >
        <div className="space-y-1 text-center">
          <h1 className="text-lg font-semibold">Leads internacionais</h1>
        ...
        {error === "config" && (
          <p className="text-sm text-destructive">
            APP_PASSWORD_INTERNACIONAL não está configurada no ambiente.
          </p>
        )}
